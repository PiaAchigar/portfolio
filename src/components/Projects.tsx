      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map(project => {
            // Only local project images (root-relative /foo.png) have a .webp
            // sibling generated by scripts/optimize-images.mjs. Supabase-sourced
            // image_url values (Storage URLs, possibly with a query string) have
            // no such sibling — rendering a <source> for them would 404 with no
            // fallback to the <img>, so skip it and let the <img src> handle those.
            const webpSrc = /^\/[^?#]+\.(png|jpe?g)$/i.test(project.image_url)
              ? project.image_url.replace(/\.(png|jpe?g)$/i, '.webp')
              : null
